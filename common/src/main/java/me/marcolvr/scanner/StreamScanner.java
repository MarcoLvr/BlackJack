 ...
 * @since   1.5
 */
public final class StreamScanner implements Iterator<String>, Closeable {
    ...
    // The input source
    //MarcoLvr: Changed to InputStreamReader
     ...
     */
    private StreamScanner(InputStreamReader source, Pattern pattern) {
     ...
     *         interface
     */
    public StreamScanner(InputStreamReader source) {
     ...
     */
    public StreamScanner(InputStream source) {
     ...
     *         does not exist
     */
    public StreamScanner(InputStream source, String charsetName) {
     ...
     * @since  10
     */
    public StreamScanner(InputStream source, Charset charset) {
     ...
     */
    //MarcoLvr: changed return type to InputStreamReader
    private static InputStreamReader makeReadable(Path source, Charset charset)
            throws IOException {
    ...
    //MarcoLvr: changed return type to InputStreamReader
    private static InputStreamReader makeReadable(InputStream source, Charset charset) {
        Objects.requireNonNull(charset, "charset");
     ...
     */
    public StreamScanner(File source) throws FileNotFoundException {
     ...
     *         not found
     */
    public StreamScanner(File source, String charsetName)
     ...
     * @since  10
     */
    public StreamScanner(File source, Charset charset) throws IOException {
        this(Objects.requireNonNull(source), charset.newDecoder());
    }

    private StreamScanner(File source, CharsetDecoder dec)
            throws FileNotFoundException
    {
    ...
    //MarcoLvr: changed return type to InputStreamReader
    private static InputStreamReader makeReadable(ReadableByteChannel source,
                                         CharsetDecoder dec) {
        return new InputStreamReader(Channels.newInputStream(source), dec);
    }
    ...
    private static InputStreamReader makeReadable(ReadableByteChannel source,
                                         Charset charset) {
        Objects.requireNonNull(charset, "charset");
        return new InputStreamReader(Channels.newInputStream(source), charset);
    }
     ...
     * @since   1.7
     */
    public StreamScanner(Path source)
            throws IOException
    {
     ...
     * @since   1.7
     */
    public StreamScanner(Path source, String charsetName) throws IOException {
     ...
     * @since   10
     */
    public StreamScanner(Path source, Charset charset)  throws IOException {
     ...
     */
    public StreamScanner(String source) {
        this(makeReadable(new StringReader(source)), WHITESPACE_PATTERN);
    }

    //MarcoLvr: created new method to convert a StringReader to an InputStreamReader
    private static InputStreamReader makeReadable(StringReader stringReader) {
        try{
            ...
            return new InputStreamReader(new ByteArrayInputStream(b.toString().getBytes()));
        }catch (Exception e){
            e.printStackTrace();
        }
        return null;
     ...
     */
    public StreamScanner(ReadableByteChannel source) {
     ...
     *         does not exist
     */
    public StreamScanner(ReadableByteChannel source, String charsetName) {
     ...
     * @since 10
     */
    public StreamScanner(ReadableByteChannel source, Charset charset) {
        this(makeReadable(Objects.requireNonNull(source, "source"), charset),
                WHITESPACE_PATTERN);
    }
     ...
     * @return this scanner
     */
    public StreamScanner useDelimiter(Pattern pattern) {
        modCount++;
        delimPattern = pattern;
        return this;
    }
     ...
     * @return this scanner
     */
    public StreamScanner useDelimiter(String pattern) {
        modCount++;
     ...
     * @return this scanner
     */
    public StreamScanner useLocale(Locale locale) {
     ...
     */
    public StreamScanner useRadix(int radix) {
     ...
     * If the scanner does not advance past any input the method
     * checks the InputStreamReader with the ready() method.
     *
    ...
    public boolean hasNext() {
        ...
        while (!sourceClosed) {
            ...
            //Marcolvr: changed readInput() to source.ready() which is a non-blocking method
            try{
     ...
     * @throws IllegalStateException if this scanner is closed
     */
    public StreamScanner skip(Pattern pattern) {
        ensureOpen();
        if (pattern == null)
            throw new NullPointerException();
        clearCaches();
     ...
     * @throws IllegalStateException if this scanner is closed
     */
    public StreamScanner skip(String pattern) {
